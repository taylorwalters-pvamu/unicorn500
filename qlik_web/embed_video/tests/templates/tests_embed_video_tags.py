...
class EmbedTestCase(TestCase):
    ...
    def requests_mock(mocker):
        """
     A fixture to mock the requests library.
        """
        def _mock_get(url, text):
            parsed_url = urlparse(url)
            mocker.patch(
                f'requests.get.{parsed_url.scheme}.{parsed_url.netloc}.{parsed_url.path}',
                return_value=mocker.MagicMock(text=text)
        )

        return _mock_get

    def test_my_function(requests_mock, mocker):
    # Define the URL and response text
        url = 'https://example.com/api'
        response_text = 'Mocked response'

    # Mock the requests.get() function
        requests_mock(url, response_text)

    # Now, when your function makes a request to the mocked URL,
    # it will receive the mocked response instead of making a real HTTP request
